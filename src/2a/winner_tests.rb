...
class WinnerCalculatorTests < Test::Unit::TestCase
  ...
  test 'player who reaches max score first does not win' do
    winner = WinnerCalculator.get_winner(['mike 3', 'mike -1', 'andrew 3'])
    assert_equal('andrew', winner)
  end

  test 'player who reaches max score first wins after losing some points' do
  ...
  test 'both players lost scores and wins first who scored first' do
